...
## V3-RECOVERY — engineering recovered; scientific production BLOCKED

The user-reported interrupted run encountered repeated fetch/tool failures during `scripts/v3_nextstage_report.py` editing. No scientific failure or result is inferred. Recovery inspection found both intended edits fully applied; no script repair or reset was needed. Offline report and next-stage ledger generation succeeded, preserving prior evidence.

Existing next-stage evidence supersedes the old infrastructure-only access limitations below: local Qwen3 processed the fixed sample (95 content-bearing calls plus 17 blocked records), but failed its validation gate (8/12 synthetic control expectations). See `results/v3/next-stage/experiment_log.jsonl` and `reports/SEMANTIC_VALIDATION_NEXT_STAGE.md`.

During recovery, one access-only CLI invocation returned READY after an error attempt. Separate structured diagnostics accepted one Afrikaans refusal control; the next isiZulu control timed out at 45 seconds. Stopped without a new fixed-sample run. No full rescore, translation retry or intervention occurred; corrected ASR remains null. These are three CLI invocations across two purposes, not three semantic validation records. Only one control has a completed judgment. Automated validation is not native-speaker validation.

No interruption start time is invented. Observed probe/recording times are in `results/v3/recovery/remote_access_probe.json` and `results/v3/recovery/experiment_log.jsonl`; complete recovery/QC account: [V3_RECOVERY.md](V3_RECOVERY.md). Historical entries below retain their original scope and artifact times.
