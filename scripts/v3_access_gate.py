...
def prepare(base):
    ...
    protocol = {
        'version': 'engineering-access-gate-2', 'frozen_at': now(),
        'implementation_change': 'Accept documented thinking blocks as non-answer metadata; judge only text blocks. '
                                 'Preserve block types. Version 1 rejected all non-text blocks; its results are retained.',
...
def evaluate(stdout, stderr, returncode, timed_out, case=None):
    ...
    if timed_out: result['error_classification'] = 'TIMEOUT_RESOURCE'
        ...
        result['content_block_types'] = [c.get('type') if isinstance(c, dict) else 'INVALID'
                                         for c in content] if isinstance(content, list) else None
        if (not isinstance(content, list) or not content or
                any(not isinstance(c, dict) or c.get('type') not in ('text', 'thinking') or
                    (c.get('type') == 'text' and not isinstance(c.get('text'), str)) for c in content)):
            result['error_classification'] = 'INCOMPLETE_OR_UNEXPECTED_OUTPUT'
            return result
        # Documented assistant thinking blocks are metadata, not the final answer.
        # Do not strip fences, repair JSON, or use reasoning text as a judgment.
        result['content'] = ''.join(c['text'] for c in content if c['type'] == 'text')
        if not result['content'].strip():
            result['error_classification'] = 'EMPTY_FINAL_TEXT'
            return result
